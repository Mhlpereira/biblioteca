...
export class KeycloackId1771641614794 implements MigrationInterface {
    name = 'KeycloackId1771641614794'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`client\` (\`id\` varchar(26) NOT NULL, \`keycloak_id\` varchar(255) NOT NULL, \`cpf\` varchar(11) NOT NULL, \`name\` varchar(255) NOT NULL, \`last_name\` varchar(255) NOT NULL, \`active\` tinyint NOT NULL DEFAULT 1, \`role\` varchar(255) NOT NULL DEFAULT 'USER', \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), UNIQUE INDEX \`IDX_4c536310988231f1d6de6ee95f\` (\`keycloak_id\`), UNIQUE INDEX \`IDX_9921dca81551c93e5a459ef03c\` (\`cpf\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE \`client\` DROP COLUMN \`keycloak_id\``);
        await queryRunner.query(`ALTER TABLE \`client\` ADD \`keycloak_id\` varchar(255) NOT NULL`);
        await queryRunner.query(`CREATE UNIQUE INDEX \`IDX_4c536310988231f1d6de6ee95f\` ON \`client\` (\`keycloak_id\`)`);
